...
\begin{atmTheor} \cite{Katok}
    The canonical p-adic expansion (see Definition \ref{canonical}) represents a rational number if and only if it is eventually periodic to the left.  
\end{atmTheor}
\bigskip
(WTS: Rationality $\implies$ Periodicity and Periodicity $\implies$ Rationality)
\begin{atmProof}\footnote{As a convention, we intend to keep most proofs terribly informal throughout, but refer one to the bibliography for appropriate constructions and details.}\\
$\implies$\\
\begin{enumerate}
...
The reduction to canonical form leads to an addition/subtraction system of carries similar  to $\R$ but starting from right to left (see Figure \ref{3addition}).  
...
\end{example}
\begin{example}
  ``To divide is to contain" for Dedekind domains, a ``prime factorizable" integral domain (see Definition \ref{intdomain}).  All PIDs (see Definition \ref{PID}) are Dedekind domains.   
\end{example}
\begin{defi}\label{PID}
    \colorbox{GreenYellow}{Principal Ideal Domain (PID)}\\  
    A PID is an integral domain (see Definition \ref{intdomain}) in which every ideal is principal.  
\end{defi}
\begin{example}
    $\Z$
\end{example}
...
\begin{atmProp}\cite{Katok}
    The ring $\Z_p$ is an integral domain (see Definition \ref{intdomain}).  
\end{atmProp}
\begin{atmProof}
...
\end{atmProof}
\begin{corr}
     The ring $\Z_p$ has a unique maximal ideal (see Definition \ref{maxideal}), namely
     \[p\Z_p = \Z_p /\Z_p^\times.\]
\end{corr}
\begin{atmProof}
...
\end{re}
\begin{atmProp}\cite{Katok}
  The ring $\Z_p$ is  a PID (see Definition \ref{PID}).  More precisely, its ideals are the principal ideals (see Definition \ref{PI})  $\{0\}$ and $p^k\Z_p$ for all $k \in \N$.  
\end{atmProp}
\begin{atmProof}
...
\begin{atmTheor} 
    ...
    \end{enumerate}
\end{atmTheor}
(WTS:  The sequence $\{x_0^{p^n}\}$ is Cauchy and converges to the desired limit in $\Z_p$. We  use Lemma \ref{lem1.1}  to show that the limit exists for all $x \in \Z_p$ and is defined by $x_0$ to prove 1) and swiftly proceed to prove 2) and 3). )
\bigskip
\begin{atmProof} 
\begin{enumerate}
...
\end{enumerate}
    We use Lemma \ref{lem1.1} to prove existence of a limit for all $x \in \Z_p$, \textbf{defined by digit $x_0$ of $x$}.     
